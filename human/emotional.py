...
# ------------------------------------------------------------------------------
# 2) DATASET CLASS (NOW FORCES GRAYSCALE -> RGB)
# ------------------------------------------------------------------------------
class HFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # *** Always convert to 3-channel RGB if not already ***
        img = img.convert("RGB")

        if self.transform:
            img = self.transform(img)

        # Convert list -> FloatTensor
...
def main():
    ...
    # ---------------------------
    DATA_DIRS = [
        "human/archive",            # Original path
        "fer2013",                  # 1st additional dataset
        "mma-facial-expression",    # 2nd additional dataset
        "testingemotion"            # 3rd additional dataset
    ]
